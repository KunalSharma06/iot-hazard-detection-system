(() => {
  ...
  const AlertHistory = {
    ...
    init() {
      ...
      this.updatePanel();
      this.updateBadge();
      this.updateStats(); // Add this line to show correct alert count
    },
  };
  ...
  function showRoomsPage() {
    currentRoom = null;
    localStorage.removeItem("selectedRoom"); // Clear saved room
    DetailPage.reset();
  ...
  function showDetailPage(roomId) {
    currentRoom = roomId;
    localStorage.setItem("selectedRoom", roomId); // Save room to localStorage
